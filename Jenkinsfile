                ...
                parallel (
                    "agent1" : {
                            sshagent (credentials: ['jenkins']) {
                                ...
                                sh 'ssh -o StrictHostKeyChecking=no jenkins@192.168.0.28 "if [ -d /var/www/vhosts/test/storage ]; then echo 'storage exists'; else mkdir /var/www/vhosts/test/storage; chmod -R 775 /var/www/vhosts/test/storage; fi"'
                    ...
                    "agent2" : {
                            sshagent (credentials: ['jenkins']) {
                                ...
                                sh 'ssh -o StrictHostKeyChecking=no jenkins@192.168.0.31 "if [ -d /var/www/vhosts/test/storage ]; then echo 'storage exists'; else mkdir /var/www/vhosts/test/storage; chmod -R 775 /var/www/vhosts/test/storage; fi"'
